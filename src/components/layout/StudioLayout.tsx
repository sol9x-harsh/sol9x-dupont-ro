...
const WORKFLOW_STEPS = NAV.filter(n => !n.isWorkspace);

export function StudioLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [savedAt, setSavedAt] = useState<string | null>('14:32 UTC');
  const [collapsed, setCollapsed] = useState(false);

  const currentIndex = NAV.findIndex((n) => pathname.startsWith(n.id));
  const activeNav = currentIndex !== -1 ? NAV[currentIndex] : NAV[0];
  
  const workflowIdx = WORKFLOW_STEPS.findIndex(n => pathname.startsWith(n.id));
  const isWorkflowStep = workflowIdx !== -1;
  
  const prev = isWorkflowStep && workflowIdx > 0 ? WORKFLOW_STEPS[workflowIdx - 1] : (isWorkflowStep && workflowIdx === 0 ? NAV[0] : null);
  const next = isWorkflowStep && workflowIdx < WORKFLOW_STEPS.length - 1 ? WORKFLOW_STEPS[workflowIdx + 1] : (pathname.startsWith('/studio/profile') ? WORKFLOW_STEPS[0] : null);
    ...
    <TooltipProvider delayDuration={0}>
      <div className="flex h-screen w-full bg-slate-50 overflow-hidden" role="application">
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-2xl bg-slate-900 flex items-center justify-center shadow-xl shadow-blue-900/10 relative overflow-hidden group cursor-pointer" onClick={() => router.push('/projects')}>
                <div className="absolute inset-0 bg-gradient-to-br from-primary to-primary-glow opacity-100 group-hover:scale-110 transition-transform duration-500" />
          ...
          <div className="px-4 mb-2">
            {!collapsed && <span className="px-2 text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">Workflow Steps</span>}
          </div>

          <nav className={cn('flex-1 overflow-y-auto scrollbar-premium transition-all duration-300 space-y-1', collapsed ? 'px-3 mt-2' : 'px-4')}>
            {WORKFLOW_STEPS.map((n, i) => {
              const Icon = n.icon;
              const isActive = pathname.startsWith(n.id);
                  ...
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={cn(
                        ...
                        isActive ? 'bg-slate-50 text-primary shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-50/50 hover:text-slate-900',
                      )}
                      style={{ animationDelay: `${400 + i * 50}ms` }}
                    >
                      {isActive && !collapsed && <div className="absolute left-[-1px] top-1/2 -translate-y-1/2 w-[3px] h-6 rounded-r-full bg-primary" />}
                      <div
                        className={cn(
                          'flex items-center justify-center shrink-0 transition-all duration-300',
                          collapsed ? 'w-10 h-10' : 'w-10 h-10',
                          isActive ? 'bg-primary/10 text-primary rounded-xl' : isComplete ? 'bg-success/5 text-success rounded-xl' : 'bg-slate-50 text-slate-400 rounded-xl group-hover:bg-slate-100',
                        )}
                      >
                      ...
                      {!collapsed && (
                        <div className="flex-1 text-left min-w-0">
                          <div className={cn('font-bold text-sm leading-tight truncate transition-colors', isActive ? 'text-slate-900' : 'text-slate-600')}>{n.label}</div>
                          <div className="text-[10px] mt-0.5 text-slate-400 font-medium truncate">{n.shortDesc}</div>
                        </div>
                      )}
                      {isActive && !collapsed && <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />}
                    </button>
                  </TooltipTrigger>
                  {collapsed && (
          ...
          <div className="mt-auto pt-4">
            <div className="px-4 mb-2">
              {!collapsed && <span className="px-2 text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">Workspace</span>}
            </div>
            <div className={cn('px-4 space-y-1', collapsed && 'px-3')}>
              <button
                onClick={() => router.push(NAV[0].id)}
                className={cn(
                  'w-full flex items-center rounded-xl transition-all duration-300 relative group',
                  collapsed ? 'h-[52px] justify-center' : 'px-3 py-3 gap-3',
                  pathname.startsWith(NAV[0].id) ? 'bg-slate-50 text-primary shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-50/50 hover:text-slate-900',
                )}
              >
                <div
                  className={cn(
                    'w-10 h-10 flex items-center justify-center shrink-0 transition-all duration-300 rounded-xl',
                    pathname.startsWith(NAV[0].id) ? 'bg-primary/10 text-primary' : 'bg-slate-50 text-slate-400 group-hover:bg-slate-100',
                  )}
                >
                ...
                {!collapsed && (
                  <div className="flex-1 text-left min-w-0">
                    <div className={cn('font-bold text-sm leading-tight truncate', pathname.startsWith(NAV[0].id) ? 'text-slate-900' : 'text-slate-600')}>Settings</div>
          ...
          <div className="p-4 mt-2">
            ...
            >
              <div className="relative w-10 h-10 rounded-xl bg-gradient-to-br from-primary/90 to-primary flex items-center justify-center shrink-0 shadow-lg shadow-primary/20">
                <span className="font-display font-bold text-white text-xs">JD</span>
              </div>
              {!collapsed && (
                <div className="relative flex-1 min-w-0">
                  <div className="text-xs font-bold text-slate-900 truncate">John Doe</div>
                  <div className="text-[10px] text-slate-400 font-bold truncate uppercase tracking-wider">SOL9X Pvt Ltd</div>
                </div>
              )}
              {!collapsed && (
        ...
        <div className="flex-1 flex flex-col min-w-0 transition-all duration-500 ease-premium">
          <header className="h-[64px] shrink-0 bg-white border-b border-slate-100 flex items-center px-8 relative z-10" role="banner">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <button onClick={() => router.push('/projects')} className="text-[11px] font-bold text-slate-400 uppercase tracking-widest shrink-0 hover:text-primary transition-colors cursor-pointer">Projects</button>
              <ChevronRight className="w-3.5 h-3.5 text-slate-300" />
              <h1 className="font-display font-bold text-slate-900 truncate text-base tracking-tight">Chennai SWRO - Desalination Plant</h1>
            </div>
            <div className="flex items-center gap-6">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden md:block">Saved at {savedAt}</div>
              <Button
                onClick={handleSave}
                disabled={saving}
          ...
          <main className="flex-1 overflow-auto bg-slate-50/50 scrollbar-premium" role="main">
            <div className="px-8 pt-4 pb-8 fade-up">
              {children}
            </div>
          </main>
          ...
          <footer className="h-[64px] shrink-0 border-t border-slate-100 bg-white/70 backdrop-blur-xl flex items-center px-6 gap-4 z-20">
            <button
              disabled={!prev}
              onClick={() => prev && router.push(prev.id)}
              ...
              {WORKFLOW_STEPS.map((n, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    i === workflowIdx ? 'w-10 bg-primary shadow-sm shadow-primary/20' : i < workflowIdx ? 'w-2 bg-success/60' : 'w-2 bg-slate-200',
                  )}
                />
              ))}
            </div>
            <Button
              disabled={!next && !pathname.startsWith(NAV[0].id)}
              onClick={() => next && router.push(next.id)}
