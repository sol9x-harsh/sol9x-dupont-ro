...
export function analyzeConductivityFromTDS(
  ...
  return { conductivityUsCm, estimationMethod: "tds", classification, status, message };
}
 ...
 * Cross-checks the two estimates and returns the ion-weighted value as primary
 * when both are available (more physically grounded), otherwise falls back to TDS.
 */
export function analyzeConductivityDual(
  ...
  const tdsBasedUsCm = estimateConductivityFromTDS(tdsMgL, factor);
  const ionWeightedUsCm = estimateConductivityFromIons(concentrations);

  // Choose primary estimate: ion-weighted when available, TDS-based as fallback
  const hasBoth = tdsBasedUsCm > 0 && ionWeightedUsCm > 0;
  const conductivityUsCm = hasBoth
    ? ionWeightedUsCm
    : ionWeightedUsCm > 0
    ? ionWeightedUsCm
    : tdsBasedUsCm;

  const method: ConductivityEstimationMethod = hasBoth
    ? "ion-weighted"
    : ionWeightedUsCm > 0
    ? "ion-weighted"
    : "tds";
  ...
  return {
    conductivityUsCm,
    estimationMethod: hasBoth ? "hybrid" : method,
    classification,
    status,
    message,
    tdsBasedUsCm,
